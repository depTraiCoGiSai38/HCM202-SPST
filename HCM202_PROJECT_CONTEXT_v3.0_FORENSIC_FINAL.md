...
| `05_Showcase_Slides_Script` | Showcase `.pptx` plus speaking-role script allocated minute-by-minute to 100% of members; identify who presents theory, demos the product and analyzes interaction data. | Actual file and timed script; no invented member/role. Exact named template filename is `NEED VERIFICATION`. **`SUPERSEDED RUBRIC REQUIREMENT` (recorded 18-9-2026):** the `.pptx` is **not a scored item**. The department grading instrument — `RUBRIC ASSIGNMENT BỘ MÔN — FALL 2026` (`RUBRIC BỘ MÔN v4.1`), in the lecturer-owned gradebook `HCM202_SE1810_Fall2026_Half 1` — contains no `.pptx` deliverable; slides appear only in criterion 2.1 as something the presenter must **not** read from (`không đọc slide`). The `.pptx` line survives solely as a dossier-packaging item in `SG PDF p.6`, and the lecturer's updated README of 13-9-2026 does not restate it. Do **not** create a `.pptx` to satisfy this row. Status: `NEEDS LECTURER CONFIRMATION`, non-blocking. The Showcase itself took place on **17-9-2026, Slot 4**. | `SG PDF p.6`; `HB PDF p.4` |
...
**Class code for this group: `Half1_SE1810`.** The `SE1801` above is quoted from the
Guideline's own printed example and is left exactly as printed; it is not this group's
class. Every file the group submits uses `Half1_SE1810` in the `[LỚP]` position.
